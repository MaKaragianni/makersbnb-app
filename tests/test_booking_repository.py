...
def test_create_booking(db_connection):
    ...
    repository.create(Booking(None, 1, None, 1, None, date(2026, 5, 20), date(2026, 5, 22), 'pending'))
    bookings = repository.all()
    assert len(bookings) == 2
    assert bookings[-1].space_id == 1
    assert bookings[-1].user_id == 1
    assert bookings[-1].booking_start == date(2026, 5, 20)
    assert bookings[-1].booking_end == date(2026, 5, 22)
